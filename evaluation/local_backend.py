...
def run_tests_local(  # noqa: PLR0915
    example: Any,
    generated_patch: str,
    config: Any,
    *,
    ...
    _t0 = time.time()
    logger.info("── instance %s (%d tests) ──", example.instance_id, len(test_names))
    ...
    logger.info("  total: %.1fs for %s", _elapsed, example.instance_id)
    return {
        "repo": example.repo,
