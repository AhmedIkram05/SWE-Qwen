...
Usage:
    ...
    # Full pipeline run (SWE-bench default)
    python -m data_engineering.cli run

    # Run with custom config
    python -m data_engineering.cli run \
        --swe-bench-dir data/swe_bench \
        --output data/ \
        --max-issues 1000 \
        --parallel-workers 8
...
app = typer.Typer(
    ...
    help="SWE-Qwen pipeline: ingest -> validate -> clean -> split -> golden -> version -> archive",
...
def run(  # noqa: PLR0913,B008 -- typer CLI dispatcher; Option() calls required by typer
    swe_bench_dir: Path = typer.Option(
        "data/swe_bench",
        "--swe-bench-dir",
        help="Directory for SWE-bench dataset cache",
        file_okay=False,
    ),
    ...
    max_issues: int = typer.Option(
        2000,
        "--max-issues",
        help="Max issues per repo (reserved for future use)",
        min=1,
    ),
